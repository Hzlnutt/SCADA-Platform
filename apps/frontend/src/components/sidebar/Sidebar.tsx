...
const IconAlarm = () => (
  <svg
    ...
    strokeLinejoin="round"
  >
    <path d="M12 4a6 6 0 0 0-6 6v3l-2 2h16l-2-2v-3a6 6 0 0 0-6-6" />
...
const IconReport = () => (
  <svg
    ...
    strokeLinejoin="round"
  >
    <path d="M7 3h7l5 5v13a1 1 0 0 1-1 1H7a1 1 0 0 1-1-1V4a1 1 0 0 1 1-1" />
    <path d="M14 3v5h5" />
    <path d="M9 14h6" />
    <path d="M9 17h6" />
  </svg>
...
const IconSettings = () => (
  ...
  >
    ...
    <path d="M19.4 15a7.8 7.8 0 0 0 .1-6l-2.1.6a6 6 0 0 0-2.6-1.5L14 4h-4l-.8 4.1a6 6 0 0 0-2.6 1.5L4.5 9a7.8 7.8 0 0 0 .1 6l2.1-.6a6 6 0 0 0 2.6 1.5L10 20h4l.8-4.1a6 6 0 0 0 2.6-1.5z" />
  </svg>
);
...
const IconAdmin = () => (
  <svg
    ...
    strokeLinejoin="round"
  >
    <path d="M12 3l7 3v5c0 4.4-3 8.4-7 10-4-1.6-7-5.6-7-10V6l7-3" />
    ...
    <aside className="hidden min-h-screen w-[250px] shrink-0 border-r border-[#052946] bg-[#07365f] px-4 py-5 text-white lg:block">
      <div className="mb-7 flex items-center gap-3">
        ...
        <div>
          ...
          <div className="text-xs text-[#79b6eb]">
            Industrial SCADA Platform
          </div>
        </div>
      </div>
      <div className="mb-3 text-sm font-semibold uppercase tracking-[0.18em] text-[#79b6eb]">
        Utama
      </div>
      ...
      <div className="mb-3 mt-7 text-sm font-semibold uppercase tracking-[0.18em] text-[#79b6eb]">
        Machine Pages
      </div>
        ...
        {machineGroups.map((group) => (
          <div key={group.id} className="rounded-lg border border-white/10">
            <button
              ...
              className="flex w-full items-center justify-between gap-2 px-3 py-2 text-left text-sm text-white"
            >
              <span className="flex items-center gap-2">
                <span className="text-[#9dccf5]">
                  <IconMachine />
                </span>
                {group.name}
              </span>
              <span className="text-xs text-[#9dccf5]">
                {openGroups[group.id] ? "-" : "+"}
              </span>
            </button>
            {openGroups[group.id] ? (
      ...
      <nav className="flex flex-col gap-1.5">
        ...
        {canApprove ? (
          <NavItem to="/approvals" label="Approvals" icon={<IconReport />} />
        ) : null}
        {canApprove ? (
          <NavItem to="/thresholds" label="Thresholds" icon={<IconSettings />} />
        ) : null}
        <NavItem to="/settings" label="Settings" icon={<IconSettings />} />
        {isAdmin ? (
          <NavItem to="/admin" label="Admin Panel" icon={<IconAdmin />} />
        ) : null}
      </nav>
