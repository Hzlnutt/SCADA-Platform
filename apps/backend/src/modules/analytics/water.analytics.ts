...
export const getWaterAnalytics = async (
  ...
  for (const [devId, devRecords] of recordsByDevice) {
    ...
    for (let i = 1; i < devRecords.length; i++) {
      ...
      totalM3 += diff;
      yearlyM3 += diff;
      if (isToday) todayM3 += diff;
      if (isCurrentMonth) monthlyM3 += diff;
      ...
      if (isCurrentMonth) {
        perDeviceCurrentMonth.set(
          devId,
          (perDeviceCurrentMonth.get(devId) || 0) + diff
        );
      }
    }
  }
  ...
  for (let m = 1; m <= 12; m++) {
    ...
    monthly.push({ month: monthKey, value: monthlyMap.get(monthKey) || 0 });
  }
  ...
  for (let m = 1; m <= 12; m++) {
    ...
    for (let d = 1; d <= numDays; d++) {
      ...
      daily.push({ day: dayKey, value: dailyMap.get(dayKey) || 0 });
    }
  }
